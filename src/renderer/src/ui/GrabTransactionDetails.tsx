import { MatchItem } from "@renderer/types/results";

interface Props {
  r: MatchItem;
}

const GrabTransactionDetails = ({ r }: Props) => {
  const pos = r.pos;
  const grab = r.grab;
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative">
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        {pos ? (
          <div className="space-y-3 text-sm">
            <DataRow label="Order Reference" value={pos.cusno} />
            <DataRow label="Gross Amount" value={`₱${pos.grschrg.toLocaleString()}`} />
            <DataRow
              label="Discount (POS)"
              value={`₱-${(pos.promo_amt ?? 0).toLocaleString()}`}
            ...
            <div className="pt-3 border-t border-slate-100 flex justify-between items-end">
              ...
              <span className="text-lg font-black text-indigo-700">₱{pos.balance.toLocaleString()}</span>
            </div>
          </div>
        ) : (
          <div className="h-32 flex flex-col items-center justify-center text-rose-400 italic text-sm border-2 border-dashed border-rose-100 rounded-xl">
            <span className="font-black text-[10px] uppercase tracking-tighter">No POS Data</span>
          </div>
        )}
      </div>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        ...
        {grab ? (
          <div className="space-y-3 text-sm">
            <DataRow label="Order Reference" value={grab.booking_id ?? "-"} valueClass="font-mono text-xs font-bold" />
            <DataRow label="Gross Amount" value={`₱${grab.amount?.toLocaleString() ?? "0.00"}`} />
            <DataRow
              label="Merchant Funded Discount"
              value={`₱${(grab.discount_merchant_funded ?? 0).toLocaleString()}`}
              valueClass="text-rose-500 font-bold"
            />
            <div className="pt-3 border-t border-slate-100 flex justify-between items-end">
              <span className="text-xs font-black text-emerald-600 uppercase">Grab Net Sales</span>
              <span className="text-lg font-black text-emerald-700">
                ₱{(grab.net_sales ?? 0).toLocaleString()}
              </span>
            </div>
          </div>
        ) : (
